...
> 请[提交 Issue](https://github.com/long-910/tmux-claude-status/issues)。
...
- `~/.tmux.conf` 中的 `tmux-claude-status` 配置块
- `~/.claude/settings.json` 中的 Stop 钩子
- `~/.claude/tmux-claude-status.json`（配置文件）
...
编辑 `~/.claude/tmux-claude-status.json`：
